...
Os modelos de regressão são ferramentas estatísticas fundamentais para entender e quantificar a relação entre variáveis. Eles são amplamente utilizados em diversas áreas, como ciências sociais, biológicas e econômicas, para analisar como uma variável de interesse (a variável resposta) é influenciada por outras variáveis (variáveis explicativas).
...
\end{equation}
\noindent
Portanto, $\mathrm{I\!E}(Y) = \mu = \alpha\sigma$ e $\mathrm{Var}(Y) = \alpha\sigma^2$. Notação: $Y \sim \mathcal{G}(\alpha,\sigma)$. Nesse caso, dizemos que $Y$ segue uma distribuição gama de parâmetros de forma $\alpha$ e escala $\sigma$. A seguir, temos a densidade:
...
(b<-summary(mod2))
```
Observe que, nesse caso, podemos extrair a estimativa de $\phi$ da seguinte maneira:
```{r echo=T, message=FALSE, warning=FALSE,  warning=FALSE}
1/b$dispersion
```
...
A previsão dos valores ajustados para três modelos de regressão com resposta gama são plotados junto com os dados originais. Cada modelo (mod1, mod2 e mod3) é ajustado com base nos dados fornecidos e, em seguida, usado para prever os valores ajustados para uma faixa de valores de x. As curvas de ajuste são representadas por linhas tracejadas azuis nos gráficos. Esses gráficos permitem uma avaliação visual da qualidade do ajuste de cada modelo em relação aos dados originais.
